...
public class PhoneBook implements IPhone{
	...
	public boolean removePhoneByName(String name) {
		...
		ArrayList<Phone> listByName = this.searchPhone(name, name);
	...
	public boolean removePhoneByPhone(String phoneNumber) {
		...
		// 	}
		// }
		ArrayList<Phone> listDanhBa = this.searchPhone(phoneNumber, phoneNumber);
		if(listDanhBa.size() > 0){
			for(Phone p: listDanhBa){
				this.danhBa.remove(p);
			}
			return true;
		}
	...
	@Override
	public ArrayList<Phone> searchPhone(String name, String phoneNumber) {
		// minh luu
